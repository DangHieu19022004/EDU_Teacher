  ...
  const handleSendNow = async () => {
    ...
    try {
      for (const email of recipients) {
        ...
        // Bước 1: Lưu vào lịch gửi trước với status "sent"
        ...
        const scheduleResponse = await fetch(`${BASE_URL}contact/schedule_email/`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${user?.uid}`,
        ...
        if (scheduleData.error) {
          Alert.alert("Lỗi", `Không thể lưu lịch gửi email cho ${student.name}: ${scheduleData.error}`);
          continue;
        }

        // Bước 2: Gửi email ngay sau khi lưu lịch
        ...
        if (sendData.error) {
          Alert.alert("Lỗi", `Gửi email thất bại cho ${student.name}: ${sendData.error}`);
        } else {
          const newEmail: ScheduledEmail = {
            id: scheduleData.email_id || Date.now().toString(),
            subject: generatedSubject,
            recipients: email,
            message: generatedMessage,
            scheduledDate: now,
      ...
      Alert.alert("Thành công", "Đã lưu lịch và gửi email ngay cho các học sinh được chọn");
      resetForm();
    } catch (err) {
