        ...
        result.ocr_data = result.ocr_data.map((row: any) => {
          ...
          ['hk1', 'hk2', 'cn'].forEach(key => {
            if (row[key] && typeof row[key] === 'string') {
              ...
              if (lowerValue === 'dat' || lowerValue === 'd') {
                row[key] = '10';
              } else if (lowerValue === 'khong dat' || lowerValue === 'kd') {
                row[key] = '0';
              }
            }
          });
  ...
  const handleNext = async () => {
    ...
    try {
          ...
          const subjectsFromThisImage = results.flatMap(result =>
            result.ocr_data.map((row: any) => {
              ...
              const processScore = (score: string) => {
                ...
                if (lowerScore === 'dat'|| lowerScore === 'd') return '10';
                if (lowerScore === 'khong dat' || lowerScore === 'kd') return '0';
                return score;
              };
      ...
      const studentDataWithOCR: StudentItem = {
        ...
        name: studentInfoExtracted.name|| '',
